...
| **注入可视化** | 对话输入框上方折叠条，实时展示本次注入的 Top-N 记忆——可展开/刷新，验证“自动想起”是否生效 |
...
输入框上方的**“本次注入的记忆”**折叠条展示当前会话按工作区+全局计算的注入列表（标题含条数与字符占用，展开见 `-[kind] 内容 #tag (xN)`），点“刷新”即按当前设置重取，关闭注入时显示“注入已关闭”。
