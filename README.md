...
> 结论先行：**个人记忆插件用 json 存储领域足够，不引入数据库（SQLite / LevelDB 等）。** 依据是 1 万条真实库的实测数据，不是估算。

**实测**（10,000 条 / 4.55 MB 真实 json 库）：启动加载 65 ms，每轮模型请求的注入候选 1.7 ms，关键词检索 4.3 ms，单次保存（全文件原子重写）45 ms——万级全路径无感；而个人使用的真实负载是几百～几千条（存到 1 万条意味着每天存 20 条连存一年半）。

**外推**（线性增长）：5 万条 ≈ 23 MB、保存约 0.2 s；10 万条 ≈ 46 MB、保存约 0.5 s——相当于每天存 50 条连存五年，远远超出个人记忆插件场景。

**为什么不引入数据库**：

1. **实测余量超过两个数量级**——真实负载千级，实测万级无感；
2. **换库成本是真实的**：原生依赖（跨平台二进制、构建链）、存量数据迁移、失去 json 的人类可读（直接 cat / 编辑 / 备份）——为"个人用户几乎到不了的规模"买单，性价比为负；
3. **json 是特性不是妥协**：零依赖、装完即用、文件即数据，这正是本插件区别于通用记忆服务的定位；
4. **风险已管理，信号线明确**：若未来真实用户报告问题，升级信号是**记忆量达数万条或文件超过 20 MB**（此时保存延迟开始可感知），届时与语义召回（embedding 检索）一并规划索引式后端——在那之前不做架构预支。
