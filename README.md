...
| 功能 | 说明 |
| --- | --- |
| **4 个记忆工具** | `memory_save` 保存 / `memory_search` 检索 / `memory_forget` 删除 / `memory_restore` 恢复——模型对话中直接可调 |
| **按需召回注入** | 每次模型请求前，从用户提问抽 query → BM25F 评分 → 相关记忆追加进对话（只注入相关的，非广播式） |
| **自动捕获** | 用户说「记住：xxx / 请记住 xxx」等句式自动落库（句式与按会话上限可配置） |
| **捕获确认** | 真存上之后，下一轮提示词带「已自动捕获」让模型向用户确认「已记住 ✅」——失败不报假确认 |
...
- **敏感标记**：账号/密码/API key 等凭据可标记「敏感」——自动召回永远排除，只在管理面板或 `memory_search` 显式查时才出现
...
- **设置卡片**：注入开关/条数/长度、捕获句式/上限、删除模式——保存即时生效，卡片里有回收站与注入命中率统计
...
- **纯记忆管理**：点原点开面板，搜索 / 列表 / 编辑 / 删除 / 复制 / 底部快记；列表项与快记都有「🔒 敏感」开关，标记后自动召回不再带它
