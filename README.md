...
- **死代码清理**：移除已废弃的 embedding 体系（`embedding.ts`、`hasDeepSeekKey`、混合向量评分路径），host/client 产物各瘦身约 0.2KB
- **host 路由补测**：路由层新增 14 条单测（参数校验 / JSON body / 跨域 / 400 与 500 分流），测试总数 **81 → 95**
- **Dock 深拆**：管理面板拆成 面板容器 / 记忆 tab / 召回历史 tab / 召回气泡，`GlobalDock` **756 → 201 行**，记忆与召回各自状态自洽
