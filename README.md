<p align="center">
  <img src="src/client/assets/elephant-circle.png" width="192" alt="dsh-echo-memory logo - Echo 回音象" />
</p>
...
> 吉祥物 **Echo 回音象** 的落点 —— 右下角 44px 可拖动**原点**，点开即纯记忆管理。

- **常驻原点**：44px 圆形象标，可拖动到任意位置、双击复位，2px 白描边 + 阴影，亮/暗都醒目
- **瞬态召回**：仅相关时右下角冒泡 6s Toast（标题 13px / 正文 12px / 12px 圆角），无召回时不占位
- **纯管理**：点原点开 `380×480` 面板，搜索（即时过滤）/ 列表（最近 20 条）/ ✎ 编辑 / 🗑 删除 / ⎘ 复制 / 底部“记住：回车保存”，无杂项
- **跟随系统**：`var(--dsw-alias-bg-layer-2)` + `blur(12px)`，亮/暗自动跟
