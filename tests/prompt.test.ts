...
function provider(feed: CaptureFeed): (context: AssembleContext) => string {
  return memoryContextText(feed)
}
...
test('捕获确认段：本会话条目转述一次后消失', () => {
  ...
  const text = provider(feed)(context('s1'))
  assert.match(text, /\[记忆确认\] 刚刚已自动捕获 1 条记忆：「部署走 systemd」/)
  assert.match(text, /已记住 ✅/)
  // 已消费：再次组装不再出现
  assert.equal(provider(feed)(context('s1')), '')
})

test('确认段只回显给捕获发生的会话', () => {
  const { feed } = setup()
  feed.push({ sessionId: 's1', content: '部署走 systemd' })
  const other = provider(feed)(context('s2'))
  assert.equal(other, '')
  // 本会话的条目未被他人消费
  const self = provider(feed)(context('s1'))
