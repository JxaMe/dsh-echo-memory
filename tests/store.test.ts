...
test('save：sensitive 标记落盘，缺省 false', async () => {
  const store = makeStore()
  const out1 = await store.save({ workspace: '/w', content: '账号密码 abc 123' }, 1000)
  const out2 = await store.save({ workspace: '/w', content: '普通笔记' }, 1001)
  const rec1 = store.search({ query: '账号' })[0]
  const rec2 = store.search({ query: '普通' })[0]
  assert.ok(rec1 && rec2)
  assert.equal(rec1.record.sensitive, false) // 未显式给 → false（undefined 等同）
  assert.equal(rec2.record.sensitive, false)
  await store.update(out1.id, { sensitive: true }, 2000)
  const after = store.search({ query: '账号' })[0]
  assert.ok(after)
  assert.equal(after.record.sensitive, true)
  assert.equal(after.record.updatedAt, 2000)
  // 回转 false
  await store.update(out1.id, { sensitive: false }, 2001)
  const back = store.search({ query: '账号' })[0]
  assert.ok(back)
  assert.equal(back.record.sensitive, false)
})

test('save：sensitive 强化时保留新标记', async () => {
  const store = makeStore()
  const first = await store.save({ workspace: '/w', content: 'API key sk_abc', sensitive: true }, 1000)
  const second = await store.save({ workspace: '/w', content: 'API key sk_abc', sensitive: false }, 2000)
  assert.equal(second.existed, true)
  const rec = store.search({ query: 'sk_abc' })[0]
  assert.ok(rec)
  assert.equal(rec.record.sensitive, false) // 二次写入显式取消敏感 → 跟随新输入
})

test('rankedForInjection：敏感记忆不自动注入，但仍可 search 检索', async () => {
  const store = makeStore()
  await store.save({ workspace: '/w', content: 'VPS 密码 secret123', sensitive: true }, 1000)
  await store.save({ workspace: '/w', content: '普通部署笔记', sensitive: false }, 1001)
  const injected = store.rankedForInjection('/w', 8, 2000).map(c => c.record.content)
  assert.ok(!injected.includes('VPS 密码 secret123'))
  assert.ok(injected.includes('普通部署笔记'))
  // 手动检索仍可查（仅手动可查）
  const hit = store.search({ query: 'secret123' })[0]
  assert.ok(hit)
  assert.equal(hit.record.sensitive, true)
})
